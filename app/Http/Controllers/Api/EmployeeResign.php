...
class EmployeeResign extends Controller
{
    ...
    public function employee_export_resign(Request $request)
    {
        ...
        $selected_data = $this->filter_employee_resign($request);
        $offices = $request->office; // Current year (e.g., 2024)
        $bulan = $request->bulan;
        $tahun = $request->tahun;

        $fileName = 'Data_Resign_Karyawan' . '_' . $offices . '-' . $bulan . '-' . $tahun . '.xlsx';

        return Excel::download(new EmployeeResignExport($selected_data, $offices, $bulan, $tahun), $fileName);
    }
    ...
    public function filter_employee_resign(Request $request)
    {
        ...
        if ($offices && $bulan && $tahun) {
            $employee_resign = DB::table('v_employee_resign')->where('location_name', $offices)->whereRaw('MONTH(resign_date) = ?', [$bulan])->whereRaw('YEAR(resign_date) = ?', [$tahun])->get();
        }
        ...
        $sidebar_menu = $master_menus['sidebar_menu'];
        $grouped_sub_menu = $master_menus['grouped_sub_menu'];
        return view('layouts.admin_views.employee_resign.employee_resign_main', compact('employee_resign', 'grouped_sub_menu', 'bulan', 'tahun', 'months', 'years', 'sidebar_menu', 'office', 'offices'));
    }

    public function download_employee_resign_pdf(Request $request)
    {
        $offices = $request->office;
        $year = date('Y');
        $bulan = $request->bulan;
        ...
        $fileName = 'Data_Karyawan_Resign_' . $offices . '_' . $bulan . '_' . $year . '.pdf';
        $pdf = Pdf::loadView('layouts.pdf.employee_resign_pdf', [
            'employee_resign' => $employee_resign
        ]);
