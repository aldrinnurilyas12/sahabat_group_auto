...
class MasterVehicleData extends Controller
{
    ...
    public function vehicle_export_data(Request $request)
    {
        ...
        $status_vehicle = $request->status_vehicle; // Current year (e.g., 2024)

        $fileName = 'Data_Unit' . '_' . $location_unit . '_' . $status_vehicle . '.xlsx';

        return Excel::download(new VehicleUnitExport($selected_data, $location_unit, $status_vehicle), $fileName);
    }

    public function download_vehicle_pdf(Request $request)
    {
        $selectedStatus = $request->status_vehicle;
        $selectedLocation = $request->location_unit;
        $year = date('Y');
        ...
        // Generate PDF
        $pdf = Pdf::loadView('layouts.pdf.vehicle_pdf', [
            'vehicle' => $vehicle,
        ]);
