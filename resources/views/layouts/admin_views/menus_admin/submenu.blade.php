...
<body>
                    ...
                    <div class="card-header py-3">
                        ...
                        <div style="display: flex; gap:10px;" class="component-btn">
                            ...
                            @if ($submenu->isNotEmpty())
                                <a href="{{ route('submenu_create', ['id' => $submenu->first()->parent_id]) }}"
                                    class="btn btn-primary">
                                    <i class="fas fa-plus-circle"></i>&nbsp;Tambah Submenu
                                </a>

                                <a href="{{ route('submenu_export', $submenu->first()->parent_id) }}"
                                    class="btn btn-success">
                                    <i class="fas fa-file-excel"></i>
                                    &nbsp; Download
                                </a>
                            @else
                                <a href="{{ route('submenu_create', $submenu_parent->first()->id) }}"
                                    class="btn btn-primary">
                                    <i class="fas fa-plus-circle"></i>&nbsp;Tambah Submenu
                                </a>

                                <a href="{{ route('submenu_export', $submenu_parent->first()->id) }}"
                                    class="btn btn-success">
                                    <i class="fas fa-file-excel"></i>
                                    &nbsp; Download
                                </a>
                                        ...
                                        <tr style="width: 200px;">
                                            ...
                                            <td>
                                                <div style="display:flex; justify-content:center;gap:8px; "
                                                    class="action">
                                                    <a href="{{ route('submenu_edit', ['id' => $sub->id]) }}"><i
                                                            class="fas fa-edit"></i></a>
                                                    <a style="size: 12px;" href="#" data-toggle="modal"
                                                        data-target="#deleteSubMenu{{ $sub->id }}"><i
    ...
    @foreach ($submenu as $sub)
        <div class="modal fade" id="deleteSubMenu{{ $sub->id }}" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalLabel{{ $sub->id }}" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel{{ $sub->id }}">Hapus Data Menu Utama:
                            {{ $sub->menu_name }}</h5>
