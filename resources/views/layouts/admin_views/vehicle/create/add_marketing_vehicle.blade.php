            ...
            <div id="content">
                ...
                <div style="display: flex; gap:50px;flex-wrap:wrap;" class="form-group-content">
                    ...
                    <form style="width: 60%;" class="form_input" method="POST"
                        action="{{ route('email_marketing_vehicle.store') }}" enctype="multipart/form-data">
                        @csrf
                        @foreach ($vehicle_data as $vehicle)
                            ...
                            <input hidden type="text" name="vehicle_id" class="form-control"
                                value="{{ $vehicle->id }}" readonly autocomplete="off">

                            <div class="form-group">
                                <label>VIN/NO.POL Kendaraan <span style="color: red">*</span></label>
                                <input type="text" class="form-control"
                                    value="{{ $vehicle->vehicle_registration_number }}" readonly autocomplete="off">
                            </div>

                            <div class="form-group">
                                <label>Unit Kendaraaan</label>
                                <input type="text" class="form-control" value="{{ $vehicle->unit }}" readonly
                                    autocomplete="off">
                            </div>

                            <div class="form-group">
                                <label>Harga Unit</label>
                                <input type="text" class="form-control" value="{{ $vehicle->price }}" readonly
                                    autocomplete="off">
                            </div>

                            <div class="form-group">
                                <label>Harga Kredit Unit</label>
                                <input type="text" class="form-control" value="{{ $vehicle->credit_price }}"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" placeholder="Masukan subjek email"
                                    name="subject" autocomplete="off">
                            </div>

                            <div class="form-group">
                                <label>Judul Email</label>
                                <input type="text" class="form-control" placeholder="Masukan judul email"
                                    name="title" autocomplete="off">
                            </div>

                            <div class="form-group">
                                <label>Konten Email</label>
                                <textarea class="form-control" placeholder="Masukan konten email" name="description" id="" cols="30"
                                    rows="10"></textarea>
                            </div>

                            <div class="form-group">
                                <label for="">Gunakan Link?</label>
                                <input style="margin-bottom: 10px;" type="text" class="form-control" name="link"
                                    value="{{ strtolower($vehicle->brand . '-' . $vehicle->vehicle_type . '-' . $vehicle->manufacture_year) }}"
                                    readonly autocomplete="off">
