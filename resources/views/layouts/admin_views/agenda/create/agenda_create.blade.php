...
<head>
    ...
    <title>Buat Agenda Kantor - SAHABAT GROUP AUTO ADMINISTRATOR</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div id="content">
                    ...
                    <h4 style="text-align:center;color:black;font-weight:bold;">Tambah Data Agenda</h4>

                    <div style="display: flex; gap:50px;flex-wrap:wrap;" class="form-group-content">
                        <form class="form_input" method="POST" action="{{ route('master_agenda.store') }}">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="branch" id="">
                                    ...
                                    @foreach ($branch as $br)
                                        <option value="{{ $br->id }}">{{ $br->location_name }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="form-group">
                                <label>Pemimpin Meeting</label>
                                <select class="form-control" name="meeting_leader" id="">
                                    <option value="">==== Pilih ====</option>
                                    @foreach ($meeting_leader as $lead)
                                        <option value="{{ $lead->nik }}">[{{ $lead->nik }}] -
                                            {{ $lead->name }}
                                        </option>
                                    @endforeach
                                </select>
                                <x-input-error :messages="$errors->get('meeting_leader')" class="mt-2" style="color: red;" />
                            </div>

                            <div class="form-group">
                                <label>Nama Agenda</label>
                                <input type="text" class="form-control" name="agenda_name"
                                    value="{{ old('agenda_name') }}" autocomplete="off"
                                    placeholder="Masukan Nama Agenda">
                                <x-input-error :messages="$errors->get('agenda_name')" class="mt-2" style="color: red;" />
                            </div>

                            <div class="form-group">
                                <label>Tanggal Agenda</label>
                                <input type="date" class="form-control" name="agenda_date" autocomplete="off">
                                <x-input-error :messages="$errors->get('agenda_date')" class="mt-2" style="color: red;" />
                            </div>
