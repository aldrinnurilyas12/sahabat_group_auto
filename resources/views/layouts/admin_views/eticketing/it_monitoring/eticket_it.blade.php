...
<body id="page-top">
    ...
    <div id="wrapper">
                                ...
                                <table style="font-size: 14px; color:black;" class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr style="width: 200px;">
                                            <td><?php echo $no++ ?></td>
                                            <td><div style="display:block; justify-content:center;gap:8px; " class="action">
                                                @if($ticket->approval_by_it == 'sudah konfirmasi' && $ticket->status == 'on progress')
                                                <a class="btn btn-primary" href="#" data-toggle="modal" data-target="#konfirmasiTicketStatus{{$ticket->id}}">Ubah Status</a>
                                                @elseif($ticket->approval_by_it == 'sudah konfirmasi' && $ticket->status == 'done')
                                                <a class="btn btn-secondary" href="#">Selesai</a>
                                                @else
                                                 <a class="btn btn-primary" href="#" data-toggle="modal" data-target="#konfirmasiTicket{{$ticket->id}}">Proccess</a>
                                                @endif
                                            </td>
                                            <td>{{$ticket->name}}</td>
                                            <td>{{$ticket->title}}</td>
     ...
     @foreach($eticket_data as $ticket) 
     <div class="modal fade" id="konfirmasiTicket{{$ticket->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel{{$ticket->id}}" aria-hidden="true">
         <div class="modal-dialog" role="document">
             <div class="modal-content">
                 ...
                 <form style="padding: 5px;" method="POST" action="{{route('confirmed_eticket', $ticket->id)}}" >
                     @csrf
                     @method('PUT')
                     <div style="color: black;" class="modal-body">
                        <p style="color: black;">E-Ticket Number : #{{$ticket->id}}</p style="color: black;">
                        ...
                        <div class="form-group">
                            <input class="form-control" type="text" value="sudah konfirmasi" hidden name="approval_by_it" readonly>
                        </div>  
    ...
    {{-- Modal Konfirmasi tiket selesai --}}
     @foreach($eticket_data as $ticket) 
     <div class="modal fade" id="konfirmasiTicketStatus{{$ticket->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel{{$ticket->id}}" aria-hidden="true">
         <div class="modal-dialog" role="document">
             <div class="modal-content">
                 <div class="modal-header">
                     <h5 class="modal-title" id="exampleModalLabel{{$ticket->id}}">Konfirmasi E-Ticket: {{$ticket->eticket_category}}</h5>
                 ...
                 <form style="padding: 5px;" method="POST" action="{{route('confirmed_eticket_done', $ticket->id)}}" >
                     @csrf
                     @method('PUT')
                     <div style="color: black;" class="modal-body">
                        <p style="color: black;">E-Ticket Number : #{{$ticket->id}}</p style="color: black;">
     ...
     {{-- modal show files --}}
     @foreach($eticket_data as $ticket) 
     <div class="modal fade" id="showFile{{$ticket->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel{{$ticket->id}}" aria-hidden="true">
         <div class="modal-dialog" role="document">
             <div style="padding:10px;color:black;" class="modal-content">
                ...
                <img style="width:100%;height:500px;" src="{{asset('storage/' . $ticket->attachment_files)}}" alt="">
             </div>
         </div>
     </div>
