                    ...
                    @foreach ($eticket_data as $ticket)
                        <div class="form-group-content">
                            ...
                            <form class="form_input" method="POST"
                                action="{{ route('update_eticket', $ticket->eticket_code) }}">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="eticket_category" id="">
                                        @foreach ($eticket_category as $category)
                                            ...
                                            @endphp
                                            <option value="{{ $value }}"
                                                {{ $value == $ticket->eticket_category ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                        ...
                                        @php
                                            $anotherOptions = [
                                                'Error Wi-Fi' => 'Wi-Fi Bermasalah',
                                                'Error Printer' => 'Printer Bermasalah',
                                                'Server Services' => 'Layanan Server',
                                                'Error Laptop' => 'Laptop Bermasalah',
                                            ];

                                        @endphp

                                        @foreach ($anotherOptions as $value => $label)
                                            <option value="{{ $value }}"
                                                {{ $value == $ticket->eticket_category ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
