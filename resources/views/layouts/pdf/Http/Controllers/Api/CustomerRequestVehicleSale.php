...
use App\Exports\VehicleSaleRequestExport;
...
class CustomerRequestVehicleSale extends Controller
{
    ...
    public function filter_request(Request $request)
    {
        ...
        $customer_request_sale_data = DB::table('vehicle_sale_request as vsr')
            ->select('vsr.id', 'vehicle_type', 'vb.brand_name', 'vehicle_year', 'current_km', 'vehicle_color', 'name', 'email', 'phone_number', 'status', 'sending_email', 'description', 'vsr.created_at')
            ->leftJoin('vehicle_brand as vb', 'vsr.brand_id', '=', 'vb.id')
            ->where('vsr.id', '!=', '')->orderBy('created_at', 'DESC')->get();
        ...
        return view('layouts.admin_views.customer_vehicle_sale.customer_vehicle_sale', compact('customer_request_sale_data', 'grouped_sub_menu', 'sidebar_menu', 'years', 'months', 'bulan', 'tahun'));
    }


    public function vehicle_sale_export(Request $request)
    {

        $bulan = $request->bulan;
        $tahun = $request->tahun;
        $branch = app('App\Http\Controllers\Api\LoginAdminController')->getUsers()->location_name;
        $fileName = 'Data_Permintaan_Jual_Unit_' . '-' . $bulan .  '-' . $tahun . '.xlsx';
        return Excel::download(new VehicleSaleRequestExport($bulan, $tahun), $fileName);
    }
    ...
    public function download_request_export_pdf(Request $request)
    {
        ...
        $fileName = 'Data_Permintaan_Jual_Unit_' . $bulan . '_' . $tahun . '.pdf';
        $pdf = Pdf::loadView('layouts.pdf.customer_vehicle_sale_pdf', [
            'customer_request_sale_data' => $customer_request_sale_data
        ]);
