...
class MaintenanceUnitController extends Controller
{
     ...
     */
    public function download_vehicle_maintenance_pdf()
    {
        $offices = app('App\Http\Controllers\Api\LoginAdminController')->getUsers()->location_name;
        $maintenance_data =  DB::table('maintenance_unit as mtc')
            ->select('vehicle_id', 'unit', DB::raw('SUM(cost) as total_cost'), 'status_vehicle', 'location_name')
            ->leftJoin('v_vehicle as vhc', 'mtc.vehicle_id', '=', 'vhc.id')
            ->where('location_name', app('App\Http\Controllers\Api\LoginAdminController')->getUsers()->location_name)
            ->groupBy('vehicle_id', 'unit')
            ->get();
        ...
        // Nama file PDF
        $fileName = 'Data_Perbaikan_Unit' . '-' . $offices . '.pdf';

        // Generate PDF
        $pdf = Pdf::loadView('layouts.pdf.vehicle_maintenance_pdf', [
